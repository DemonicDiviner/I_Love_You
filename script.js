...
function showRandomCompliment() {
    ...
    // Ожидаем, пока анимация исчезновения завершится
    setTimeout(() => {
        // Проверяем, остались ли еще комплименты
        if (usedCompliments.length === compliments.length) {
            alert("Все комплименты уже использованы! Сбросим список.");
            usedCompliments = []; // Сбрасываем массив использованных комплиментов
        }

        let randomIndex;
        do {
            randomIndex = Math.floor(Math.random() * compliments.length);
        } while (usedCompliments.includes(randomIndex));

        // Добавляем выбранный комплимент в использованные
        usedCompliments.push(randomIndex);
        complimentElement.textContent = compliments[randomIndex];
...
const mediaData = [
    ...
    },
    {
        ...
        comment: 'Перед зеркалом красуется'
    },
    {
        type: 'image',
    ...
    },
    {
        ...
        comment: 'С шариком на прогулке'
    },
    {
        type: 'image',
        src: 'images/photo10.jpg',
        comment: 'Хвастаешься мне футболочкой'
    },
];
